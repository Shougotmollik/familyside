...
import 'package:familyside/provider/service_provider/sp_notification_provider.dart';
...
class SpNotificationScreen extends ConsumerStatefulWidget {
  const SpNotificationScreen({super.key});

  @override
  ConsumerState<SpNotificationScreen> createState() => _SpNotificationScreenState();
}

class _SpNotificationScreenState extends ConsumerState<SpNotificationScreen> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(spNotificationProvider.notifier).fetchNotifications();
    });
  }

  Future<void> _onRefresh() async {
    await ref.read(spNotificationProvider.notifier).fetchNotifications();
  }

  Future<void> _onMarkAllAsRead() async {
    final success = await ref
        .read(spNotificationProvider.notifier)
        .markAllAsRead();
    if (mounted && !success) {
  ...
  Widget build(BuildContext context) {
    final loc = AppLocalizations.of(context);
    final notificationState = ref.watch(spNotificationProvider);
